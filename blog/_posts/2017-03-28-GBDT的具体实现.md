...
1-> n_samples = X.shape[0]
    do_oob = self.subsample < 1.0 # 若用子训练集训练，则估计out of bag error
    ...
    i = begin_at_stage
2-> for i in range(begin_at_stage, self.n_estimators): #重复T轮迭代

        # subsampling
        if do_oob:
3->         sample_mask = _random_sample_mask(n_samples, n_inbag, #确定训练集样本，每一轮都会改变
                                              random_state)
            # OOB score before adding this stage
4->         old_oob_score = loss_(y[~sample_mask], #计算原始oob error
                                  y_pred[~sample_mask],
                                  sample_weight[~sample_mask])

        # fit next stage of trees
5->     y_pred = self._fit_stage(i, X, y, y_pred, sample_weight, #初始化
                                 sample_mask, random_state, X_idx_sorted,
                                 X_csc, X_csr)

        # track deviance (= loss)
6->     if do_oob:  #计算oob_improvement_
...
9->     if monitor is not None: #监视oob情况来判断是否已经过拟合
...
* 1.训练前的初始化，以及做好输入检测
* 2.进入T轮迭代，T=n_estimators. 注意，若采用重采样进行训练，若OOB中出现过拟合，是会提前停止迭代的(由monitor决定)
- 3.确定训练样本
- 4.计算原始oob error
