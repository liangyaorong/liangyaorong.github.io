...
<br>除平均之外，叶子节点取值方式还有：分位数，对数优势比(log Odd Ratio),先验概率，零填充等等
<br>
<br>**片段二(损失函数,以平方损失为例)**
```python
class LeastSquaresError(RegressionLossFunction):
	...
	pass
```
<br>有三个方法：求平方损失;求负梯度;将预测集在该轮训练得到的结果添加到最终预测中
<br>除平方损失之外，
<br>对于Regressor还有绝对损失，Huber损失, 分位数损失
<br>对于classifier有
