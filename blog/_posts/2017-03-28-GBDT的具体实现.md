...
<br>注意，这里面有一个for循环，其实是针对分类的(二项分类与多项分类).若是分类，模型会根据标签(label)类别对y进行重构，相同的一label放在一列.在训练中对每一列分别进行训练.若是连续型的预测，很显然n_classes_ == 1,即进入一次循环就好了
<br>
