...
<br>![](http://latex.codecogs.com/gif.latex?f(\vec{x})=f(\vec{x}^{(k)}+\triangledown f(\vec{x}^{(k)})^{T}(\vec{x}-\vec{x}^{(k)}))+\frac{1}{2}(\vec{x}-\vec{x}^{(k)})^{T}\triangledown ^{2}f(\vec{x}^{k})(\vec{x}-\vec{x}^{(k)})+o(\vec{x}-\vec{x}^{(k)}))
<br>泰勒展开的实质是用多项式去逼近原函数.
<br>对于梯度下降法,它只用了一阶泰勒展开,只有一次多项式的信息,因此要找到极值点只能通过方向导数,或者说斜率来寻找.
<br>而对于二阶泰勒展开,我们多了一项,即二次多项式.
