...
2. 可以证明(用一阶泰勒展开),方向导数可表示为梯度与l的方向余弦的内积,即:
...
<br>其中,限制l0为l方向上的单位向量,因此它的模为1; cos(theta)为梯度向量与l0的夹角
...
<br>下面用两个小例题解释一下什么叫"l方向为梯度方向时":
* 例一: P0为(1,2),从P0发出一条射线l,l方向为(1,1),求l方向上的点.
解: 显然,答案为
...
解: ![](http://latex.codecogs.com/gif.latex?\triangledown f(x)=(\frac{\partial f(x)}{\partial x_{1}},\frac{\partial f(x)}{\partial x_{2}})=(2x_{1},2x_{2})=(2,4))
