...
<br>则有:
<br>![](http://latex.codecogs.com/gif.latex?y_{k}\approx H_{k+1}\delta _{k})
<br>或
<br>![](http://latex.codecogs.com/gif.latex?H_{k+1}^{-1}y_{k}\approx \delta _{k}\ \ \ \ \ \ \ \ \ \ \  (1))
...
<br>![](http://latex.codecogs.com/gif.latex?G_{k+1}=G_{k}+\alpha uu^{T}+\beta vv^{T}\ \ \ \ \ \ \ \ \ \ \  (2))
<br>由于这里面有四个未知数,我们对它一无所知.因此不可避免地我们起码需要作四个假设才能把它们找出来.
<br>回头看回待定的式子,
![](http://latex.codecogs.com/gif.latex?G_{k+1})要满足拟牛顿条件,那我们两边先乘个
...
<br>既然是数,我们也不妨添加两个假设:
* ![](http://latex.codecogs.com/gif.latex?\alpha u^{T}\cdot y_{k}=1)
* ![](http://latex.codecogs.com/gif.latex?\beta v^{T}\cdot y_{k}=1)
<br>从这两个假设中我们可以得到:
...
还需要两个假设,那我们可以大胆地令:
* ![](http://latex.codecogs.com/gif.latex? u=\delta _{k})
* ![](http://latex.codecogs.com/gif.latex? v=-G_{k}\cdot y_{k})
好啦,四个假设用光了,我们将四个假设代回公式(2),我们有:
...
![](http://latex.codecogs.com/gif.latex?G_{k})都将满足上面的三个条件.
<br>这就是DFP算法
<br>
### BFGS算法
