...
\end{align*})
<br>
<br>3. ![](http://latex.codecogs.com/gif.latex?\widetilde{\Sigma _{b}} = W'\Sigma _{b}W'), 
其中,![](http://latex.codecogs.com/gif.latex?\Sigma _{b})为原样本类间协方差矩阵
<br>![](http://latex.codecogs.com/gif.latex?\begin{align*}
proof: \\
\widetilde{\Sigma _{b}}&=\sum _{j=1}^{c}\frac{n_{j}}{n}(\widetilde{\mu_{j}}-\widetilde{\mu})(\widetilde{\mu_{j}}-\widetilde{\mu})'\\
&=\sum _{j=1}^{c}\frac{n_{j}}{n}(W'\mu_{j}-W'\mu)(W'\mu_{j}-W'\mu)'\\
&=W'[\sum _{j=1}^{c}\frac{n_{j}}{n}(\mu_{j}-\mu)(\mu_{j}-\mu)']W\\
&=W'\Sigma _{b}W
\end{align*})
<br>
<br>4. ![](http://latex.codecogs.com/gif.latex?\widetilde{\Sigma _{w}} = W'\Sigma _{w}W), 
其中,![](http://latex.codecogs.com/gif.latex?\Sigma _{w})为原样本类内协方差矩阵
<br>![](http://latex.codecogs.com/gif.latex?\begin{align*}
proof: \\
\widetilde{\Sigma _{w}}&=\sum _{j=1}^{c}\sum_{y \in \omega _{j}} (y-\widetilde{\mu_{j}})(y-\widetilde{\mu_{j}})'\\
...
<br>
<br>5. 易得最终的目标优化函数:
<br>![](http://latex.codecogs.com/gif.latex?J(W) = \frac{|W'\Sigma _{b}W|}{|W'\Sigma _{w}W|}).该值越大越好.

* 目标优化函数求解
<br>我们有了目标函数![](http://latex.codecogs.com/gif.latex?J(W)),接下来我们需要对其进行求解.
<br>注意到,对变换矩阵W扩大C倍(C>0),方向没有改变,但![](http://latex.codecogs.com/gif.latex?\widetilde{\Sigma_{w}}), ![](http://latex.codecogs.com/gif.latex?\widetilde{\Sigma _{b}})却可以因此变得任意大.
<br>因此我们可以只考虑![](http://latex.codecogs.com/gif.latex?|W'\Sigma _{w}W|=1})时的情况,即对目标函数添加了一个条件.
<br>
<br>现在我们的目标函数变成了:
<br>![](http://latex.codecogs.com/gif.latex?W = argmax\ |W'\Sigma _{b}W|)
<br>![](http://latex.codecogs.com/gif.latex?s.t.\ |W'\Sigma _{w}W|=1)
<br>
