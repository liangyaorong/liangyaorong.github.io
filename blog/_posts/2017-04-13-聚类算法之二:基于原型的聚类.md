...
<br>**K-means的优缺点**
* 优点:
1. 计算速度快(算法复杂度![](http://latex.codecogs.com/gif.latex?O(mk\cdot round))),原理简单.
* 缺点:
1. K难以确定.
2. 受初始质心影响较大.
3. 对异常值非常敏感(平均确定质心).
...
* 缺点:
1. 由于要对每个簇中样本点进行遍历来寻找中心点,因此计算复杂度![](http://latex.codecogs.com/gif.latex?O((mk+m)\cdot round))较K-means大.因此只适用于较小的样本.
...
<br>
<br>下面我们对其进行求解:
<br>当![](http://latex.codecogs.com/gif.latex?d_{ij})为欧氏距离,对中心![](http://latex.codecogs.com/gif.latex?c_{i})偏导,有
...
<br>
<br>![](http://latex.codecogs.com/gif.latex?J')对变量![](http://latex.codecogs.com/gif.latex?u_{ij})求偏导并令其为0,得到:
...
<br>
<br>![](http://latex.codecogs.com/gif.latex?\Rightarrow \frac{(-\lambda_{j})^{\frac{1}{m-1}}}{\sum_{j=1}^{c}[m\cdot d(x_{i},c_{j})]^{\frac{1}{m-1}}}=1)
<br>
<br>![](http://latex.codecogs.com/gif.latex?\Rightarrow (-\lambda_{j})^{\frac{1}{m-1}}=\sum_{j=1}^{c}[m\cdot d(x_{i},c_{j})]^{\frac{1}{m-1}}\ \ \ \ \ (5))
<br>看,我们的到了公式(5),即![](http://latex.codecogs.com/gif.latex?\lambda_{j})所需满足的条件.将公式(5)代回公式(4),我们得到:
