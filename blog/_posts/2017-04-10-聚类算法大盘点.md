...
1. 相对接近度 (Relative Closeness, RC)
<br>相对接近度衡量的是"簇间距离"与"簇内样本点间的平均距离"的比值.
<br>其数学定义为:
...
<br>它算法的具体步骤如下:
<br>步骤一: 稀疏化,得到K-最近邻图.
<br>步骤一: 利用各种算法将数据划分为大量"较小的", "簇内样本高度相似"的簇.这些算法可以是上面OPOSSUM提到的METIS算法,当然也可以是其他图划分算法.
<br>步骤二: 用凝聚层次聚类的方法(初始簇为步骤一得到的大量簇),合并相似的簇,直到没有簇可以合并.
